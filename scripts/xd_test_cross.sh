...
for NCTX in 6; do
    for NCTX_V in 4; do
        for DATASET in eurosat dtd fgvc_aircraft oxford_flowers stanford_cars oxford_pets food101 sun397 ucf101 caltech101; do
            for SEED in 1 2 3; do
                LOADEP=3
                MODEL_DIR=${FOLDER}/train_all/imagenet/shots_${SHOTS}/${TRAINER}/${CFG}/seed${SEED}
                DIR=${FOLDER}/cross_dataset/${DATASET}/shots_${SHOTS}/${TRAINER}/${CFG}/seed${SEED}
                if [ -d "$DIR" ]; then
